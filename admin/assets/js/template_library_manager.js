;jQuery(document).ready(function($) {
    ...
    const
            ...
            htwptLoadedText = NNFYTM.message.allload,
            htwptNothingFoundText = NNFYTM.message.notfound,
        ...
        let
            // Project Data
            htwptProjectData = NNFYTM.alldata,
                ...
                html += `<div class="${singleItem} col-xl-4 col-md-6 col-12">
                            <div class="htwpt-project-item ${singleItem}" data-group="${shareId}">
                                ...
                                <div class="htwpt-project-info">
                                    ...
                                    <h6 class="sub-title">${totalItem} ${htwpUcfirst(htwptDemosValue)} ${NNFYTM.message.packagedesc}</h6>
                                </div>
                            </div>
                        </div>`;
            }
        ...
        htwptProjectGrid.on('click', '.htwpt-project-item', function (e) {
            ...
            for (let i = 0; i < htwptPrintGroupData.length; i++) {
                ...
                if(isPro == '1' ){
                    $impbutton = `<a href="${NNFYTM.prolink}" target="_blank">${NNFYTM.buttontxt.buynow}</a>`;
                    $tmptitle = `<h5 class="title">${title} <span>(${NNFYTM.prolabel})</span></h5>`;
                }else{
                    $impbutton = `<a href="#" class="htwpttemplateimp button" data-templpateopt='{"parentid":"${shareId}","templpateid":"${id}","templpattitle":"${title}","message":"Successfully ${htwpUcfirst(shareId)+ ' -> ' + title} has been imported.","thumbnail":"${thumbnail}","freePlugins":"${freePlugins}", "proPlugins":"${proPlugins}","requiredtheme":"${requiredtheme}" }'>${NNFYTM.buttontxt.import}</a>`;
                            ...
                            <div class="htwpt-project-item">
                                <div class="htwpt-project-thumb">
                                    ...
                                    <div class="htwpt-actions">
                                        <a href="${demoUrl}" target="_blank">${NNFYTM.buttontxt.preview}</a>
                                        ${$impbutton}
                                    </div>
                                </div>
                                <div class="htwpt-project-info">
    ...
    */
    $('body').on('click', 'a.htwpttemplateimp', function(e) {
        ...
        var htbtnMarkuplibrary = `<a href="#" class="wptemplataimpbtn" data-btnattr='{"templateid":"${template_opt.templpateid}","parentid":"${template_opt.parentid}","templpattitle":"${template_opt.templpattitle}"}'>${NNFYTM.buttontxt.tmplibrary}</a>`;
        var htbtnMarkuppage = `<a href="#" class="wptemplataimpbtn htwptdisabled" data-btnattr='{"templateid":"${template_opt.templpateid}","parentid":"${template_opt.parentid}","templpattitle":"${template_opt.templpattitle}"}'>${NNFYTM.buttontxt.tmppage}</a>`;
        ...
        $.ajax( {
            ...
            type: 'POST',
            data: {
                action: 'nnfytmp_ajax_get_required_plugin',
